...
{
    public class AccountsPayableViewModel : BaseViewModel
    {
        private ObservableCollection<AccountsPayable> _accounts = new();
        private AccountsPayable? _selectedAccount;
        private decimal _paymentAmount;
        private string _errorMessage = string.Empty;
        private decimal _totalBalance;
        private decimal _totalOverdue;

        public ObservableCollection<AccountsPayable> Accounts
        {
            get => _accounts;
            set => SetProperty(ref _accounts, value);
        }

        public AccountsPayable? SelectedAccount
        {
        ...
        private async Task LoadAccountsAsync()
        {
            try
            {
                var accounts = await AccountsPayableService.GetAllAsync();
                Accounts = new ObservableCollection<AccountsPayable>(accounts);
            ...
            {
                ErrorMessage = $"Error al cargar cuentas por pagar: {ex.Message}";
            }
        }
        ...
        private async void ExecuteRecordPayment(object? parameter)
        {
            ...
            {
                ErrorMessage = "Debe seleccionar una cuenta por pagar.";
                return;
            }
            ...
            var result = System.Windows.MessageBox.Show(
                $"¿Está seguro de registrar un pago de L {PaymentAmount:N2} a {SelectedAccount.VendorName}?",
                "Confirmar Pago",
            ...
            if (result == System.Windows.MessageBoxResult.Yes)
            {
                try
                {
                    await AccountsPayableService.UpdatePaymentAsync(SelectedAccount.PurchaseId, PaymentAmount);
                    _ = LoadAccountsAsync();
                    PaymentAmount = 0;
